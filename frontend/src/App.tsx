...
export default function App() {
  ...
  if (conn !== 'online') return (<div className="min-h-screen flex items-center justify-center bg-medical-900"><div className="wizard-card w-full max-w-md p-8 text-center"><h1 className="text-2xl font-bold text-accent-200 mb-2">Bolus Designer</h1><p className="text-medical-500 text-sm mb-8">放疗个性化补偿器数字化设计平台</p><div className="mb-6">{conn === 'checking' ? <div className="animate-spin inline-block w-8 h-8 border-2 border-accent-400 border-t-transparent rounded-full" /> : conn === 'launching' ? <><div className="animate-spin inline-block w-8 h-8 border-2 border-accent-400 border-t-transparent rounded-full mb-3" /><p className="text-accent-300 text-sm">Starting...</p></> : <div className="text-5xl mb-3">🔬</div>}<p className="text-medical-400 text-sm mt-3">{conn === 'checking' ? '检测中...' : conn === 'launching' ? '启动中...' : 'Slicer 未连接'}</p></div><button onClick={launchSlicer} disabled={conn === 'launching' || conn === 'checking'} className="btn-primary w-full mb-3 text-base">{conn === 'launching' ? '启动中...' : '🚀 启动 3D Slicer'}</button>{launchLog && <div className="log-stream mt-4 text-left"><pre className="text-xs text-gray-300 whitespace-pre-wrap">{launchLog}</pre></div>}</div></div>);

  return (<WizardLayout currentStep={step} totalSteps={5} onNext={handleNext} onPrev={() => { setStep((s) => Math.max(s - 1, 1)); setPipeStatus('idle'); }} canNext={canNext()} isLast={step === 6} totalSteps={6} status={pipeStatus} logs={logs} slicer={slicer} slicerOnline={slicerOnline} onReconnect={launchSlicer} connecting={conn === 'launching'}>{renderStep()}</WizardLayout>);
}
